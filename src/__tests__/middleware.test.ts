...
describe("middleware", () => {
  ...
  test("secure.optional passes without auth", async () => {
    const app = new Hono<{ Variables: Variables }>();
    app.get("/test", secure.optional, (c) =>
      c.text(`user:${c.get("username") || "none"}`),
    );
    const res = await app.request("/test");
    expect(res.status).toBe(200);
  ...
  test("boardAccess rejects nonexistent board", async () => {
    ...
    const token = session.create("mwuser");
    const res = await app.request("/nonexistent-board-xyz", {
      headers: { Cookie: `session=${token}` },
    });
  ...
  test("boardAccess rejects unauthorized user", async () => {
    ...
    const res = await app.request(`/${mwBoard.slug}`, {
      headers: { Cookie: `session=${token}` },
    });
  ...
  test("boardOwner rejects non-owner", async () => {
    ...
    db.exec(
      `INSERT OR IGNORE INTO members (board_id, username, invited_by) VALUES (${mwBoard.id}, 'mwother', 'mwuser')`,
    );
